...
#undef  XX
	...
	static SSIZE_T do_io(uintptr_t fd, OriginFun fun, const char* hook_fun_name, typename IOManager::Event event, int timeout_so, Args&&...args) {
	...
	template<typename OriginFun, typename... Args>
	static bool do_socket(SOCKET fd, OriginFun fun, const char* hook_fun_name,typename IOManager::Event event, int timeout_so,IOManager::FdContext::EventContext::ptr eventCtx ,Args&&...args) {
...
#undef  XX
	...
	{
		auto ctx = IOManager::CreateEventContext();
		return  0;///do_socket(s, recvfrom_f, "recvfrom", IOManager::READ, SO_RCVTIMEO,ctx ,lpBuffers, dwBufferCount, lpNumberOfBytesRecvd, &flags, lpFrom, lpFromlen, (LPOVERLAPPED)ctx.get(), nullptr);
	}
